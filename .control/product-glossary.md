...
| Action preset | A pre-configured system or window-management action selectable from a dropdown in Settings for a specific mouse input, avoiding manual keystroke recording. | FR-32, PRD §3.16 |
...
| Low-level mouse hook | A global OS hook (WH_MOUSE_LL) installed on the dedicated hook thread to intercept auxiliary mouse buttons (WM_XBUTTONDOWN) and horizontal tilt wheel (WM_MOUSEHWHEEL) without passing them to active applications when bound. | FR-30, PRD §3.16 |
...
| Tilt wheel debounce | A time-based rate limit (150–200 ms) applied to WM_MOUSEHWHEEL messages ensuring a single physical tilt flick triggers exactly one navigation action instead of a burst of repeated triggers. | FR-31, PRD §3.16 |
