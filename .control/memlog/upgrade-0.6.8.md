...
- (event) CI on `af5fc11` caught what this pass itself missed: `verify-public-export.ps1`'s "internal requirement or process vocabulary" check has no `Allowed` entry for `.scratch/*`, only `_bmad-output/*` — 19 findings, every one inside a file this pass moved from an already-exempt path to a non-exempt one (AD-N/FR-N/BR-N references in the moved SPEC.md/story files), nothing newly written. `wdi-systematic-debugging` habit applied: read the actual CI log before acting, confirmed root cause precisely, then fixed the source. Recorded as `DEC-015` (applied) rather than patched quietly, matching `DEC-013`'s precedent for the same gate. Verified both directions: the tree passes with the migrated content, and a mutation probe (a real `AD-N` comment planted in `crates/daemon/src/`) is still caught red.
