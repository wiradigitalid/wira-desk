...
- **Typography:** Three system-font tiers are tried in order — Segoe UI Variable Text (`SegUIVar.ttf`), then Segoe UI (`segoeui.ttf`), then Tahoma (`tahoma.ttf`) — falling back to `LoadedFont::Bundled`, the renderer's own default face, when none of the three is found.
...
- **Focus visibility, known gap:** the old `eframe`-era rule promised a widened focus stroke applied uniformly to every control via `ctx.all_styles_mut`. Slint carries no equivalent global focus style, and only one control (`layout_pane.slint`'s width input) binds `has-focus` to a visible `border-color` change at all — every other interactive element's focus-visible treatment, if any, is Slint's platform default, unaudited here. This is a real gap against FR-20/FR-21's evidence, not a documentation-only fix; filed as `OQ-24` rather than asserted away.
...
- **Binds:** CAP-10
- **Prevents:** UAC prompt on every boot (a registry `Run`-key entry cannot launch an elevated process silently); `%APPDATA%` path mismatch if the task runs as SYSTEM; DLL Hijacking via the task's working directory; a stored path outliving the executable it named.
