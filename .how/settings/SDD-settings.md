...
| **AD-11a** | `Cargo.toml` specifies `slint` with `default-features = false, features = ["backend-winit", "renderer-skia", "accessibility", "compat-1-2"]`. Only `crates/settings/ui/components/key_check.slint` and `crates/settings/ui/panes/layout_pane.slint` declare `accessible-role`/`accessible-label` (8 occurrences total, across 2 of ~14 `.slint` files). `crates/settings/ui/panes/layout_pane.slint`'s width input is the only `has-focus`-bound style found anywhere in `ui/`. `crates/settings/src/theme.rs` validates and loads font bytes from `%SystemRoot%\Fonts\segoeui.ttf` via `ttf-parser`. `main.rs` calls `main_window.global::<Palette>().set_is_dark(is_dark)` once, from `theme::detect_theme()`. |
...
- **`C-KeyCheck` (`LC-shortcut-capturer`):** Correlates the daemon's report against what this window received. `KeyCheckVerdict` declares four outcomes, but only three are ever produced: `Received` (both arrived and match), `ClaimedByOtherApp` (the window's own key event arrived with no correlating hook report, within a grace timeout), and `DaemonNotRunning` — which the code deliberately answers for two different situations DEC-005's table treats as one row: the daemon genuinely not running, and the daemon running but no hook report ever correlates (an earlier hook swallowed the chord, or none has arrived yet). The fourth declared outcome, `Intercepted`, is never constructed by any code path — `[NEEDS CONFIRMATION]`, filed as `OQ-36`: whether the label "Daemon Not Running" showing while the daemon is in fact running is an accepted simplification of DEC-005's honest-silence stance, or a UX-honesty gap worth a distinct verdict.
...
All technical mechanisms, invariants (AD-1, AD-5, AD-11, AD-11a, AD-12, AD-13), and boundary failure modes are verified against the codebase and ratified, with one exception surfaced while amending AD-11a for the Slint migration: whether `accessible-role`/`accessible-label` coverage and focus-visible styling extend beyond the two `.slint` files confirmed today is `[NEEDS CONFIRMATION]`, filed as `OQ-24`.
