...
- **`C-KeyCheck` (`LC-shortcut-capturer`):** Correlates the daemon's report against what this window received. `KeyCheckVerdict` declares four outcomes, but only three are ever produced: `Received` (both arrived and match), `ClaimedByOtherApp` (the window's own key event arrived with no correlating hook report, within a grace timeout), and `DaemonNotRunning` — which the code currently answers for two different situations DEC-005's table treats as one row: the daemon genuinely not running, and the daemon running but no hook report ever correlates (an earlier hook swallowed the chord, or none has arrived yet). The fourth declared outcome, `Intercepted`, is never constructed by any code path — `[MISSING]`, confirmed by the product owner as a real gap rather than an accepted simplification, and dispositioned as `DEF-4`.
