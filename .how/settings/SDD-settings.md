...
| --- | --- |
| **AD-1** | Settings is executed as an isolated process. It never shares memory, pointers, or mutexes with the daemon. IPC to daemon uses strictly `PostMessageW(hwnd, WM_APP_RELOAD_CONFIG, 0, 0)`. |
| **AD-5** | `crates/settings/src/persistence.rs` executes `validate_config(&cfg)` → `cfg.save(path)` (atomic temp write + rename) → `signal_reload()` via `FindWindowW` / `PostMessageW`. |
| **AD-11** | `crates/settings/build.rs` compiles `ui/main_window.slint` via `slint_build::compile`. `crates/settings/src/main.rs` builds the window with `MainWindow::new()` over `i-slint-backend-winit`. `resolve_launch_intent` evaluates CLI args for `shared::ONBOARDING_FLAG` and checks disk existence via `launch_intent()`. |
| **AD-11a** | `Cargo.toml` specifies `slint` with `default-features = false, features = ["backend-winit", "renderer-skia", "accessibility", "compat-1-2"]`. `accessible-role`/`accessible-label` are declared across the pane and component markup — `key_check.slint`, `components/shortcut_row.slint`, `panes/shortcuts_pane.slint` and their siblings — and the per-row names come from `theme.rs` through `in property` bindings rather than from literals in markup (`DEC-018`). An earlier revision of this row named `panes/layout_pane.slint` as one of only two files carrying them: that file was deleted by `SPEC-4-02` when `DEC-014` retired the Layout pane, so the count and both of its examples are superseded. `crates/settings/src/theme.rs` validates and loads font bytes from `%SystemRoot%\Fonts\segoeui.ttf` via `ttf-parser`. `main.rs` calls `main_window.global::<Palette>().set_is_dark(is_dark)` once, from `theme::detect_theme()`. |
| **AD-12** | Settings depends directly on `shared` for `Config`, `Shortcut`, `config_path()`, `migrate_appdata()`, and constants (`WM_APP_RELOAD_CONFIG`, `DAEMON_WINDOW_CLASS`, `DAEMON_WINDOW_TITLE`, `ONBOARDING_FLAG`, `TASK_NAME`). |
| **AD-13** | `General` pane auto-start toggle synchronizes with `shared::Config.general.auto_start` and Task Scheduler task `WiraDesk`. Daemon owns `schtasks` creation/deletion with `/SC ONLOGON /RL HIGHEST /RU "%USERNAME%" /TR "\"<exe_path>\"" /F`. |
...
- **`C-ShellController` (`LC-settings-shell`):** Coordinates application lifecycle, one-shot theme detection at startup, pane routing, and the window's single top-level `FocusScope`. It does **not** coordinate a deterministic keyboard focus sequence, and an earlier revision of this line said it did: no `SidebarItem` and neither action button is focusable, so there is no sequence for it to order (`DEF-7`, and `FR-20` is the promise that gap breaks). What it does own is the routing of every key that no focused element consumed — see `C-KeyCheck` for the invariant that rests on it. Slint's retained-mode renderer redraws on property change, so nothing here polls for a repaint the way an immediate-mode toolkit would.
...
- **`C-KeyCheck` (`LC-shortcut-capturer`):** Correlates the daemon's report against what this window received. **That window-side signal has two structural preconditions, and both are worth stating because breaking them is silent:** first, key events reach this correlation only through the top-level `FocusScope` in `main_window.slint`, and Slint delivers a key to the focused element and bubbles it up that element's **ancestors** only. While that scope was a childless *sibling* of the pane content, every key went unreported the moment anything else took focus — a click on a percentage field was enough — and the correlation then read hook-yes / window-no, which the table below defines as another application's claim. `DEF-9` records it; `SPEC-4-05` fixed it by nesting the content inside the scope. Anyone re-arranging that shell MUST keep the scope an ancestor of the content, or this object reports a third party that does not exist. Second, live KeyCheck updates (physical modifier tracking, beat pulse) must use `sync_key_check` alone and MUST NOT trigger full `sync_model_to_ui`: full model resynchronization recreates the row data collections, which resets any in-progress typed text in a focused percentage input back to its prior committed value mid-keystroke (`DEF-13`, `SPEC-5-03`). `KeyCheckVerdict` declares four outcomes and all four are produced: `Received` (both arrived and match), `ClaimedByOtherApp` (the window's own key event arrived with no correlating hook report, within a grace timeout), `Intercepted` (the daemon is running but no hook report ever correlates — an earlier third-party LL hook swallowed the chord before Wira Desk's own hook saw it), and `DaemonNotRunning` (the daemon is genuinely not running). `DEF-4` fixed the gap where the code answered both of the last two situations with `DaemonNotRunning`.
...
1. User navigates to the **Shortcuts** pane by mouse click. Keyboard navigation to a pane is named by `FR-20` but not built — no `SidebarItem` is focusable (`DEF-7`) — so this step has one route today, not two.
...
| Deterministic focus order verification in debug builds | **Not verified** | `crates/settings/src/app.rs` (`focus_order`, `focus_order_mismatch`) | `focus_order` declares an explicit stop array per pane, but nothing checks it against the window: `focus_order_mismatch` compares that function's output to itself, and no rendered element carries `f.label()` as an accessible name. The declaration has therefore never been enforced, and the shell has no Tab order to enforce it against — `DEF-7`. An earlier revision of this row read `Verified: … debug assertion validates drawn UI sequence against declared contract`, which was the strongest claim in this table and was false. |
...
All technical mechanisms, invariants (AD-1, AD-5, AD-11, AD-11a, AD-12, AD-13), and boundary failure modes are verified against the codebase and ratified, with two exceptions.
