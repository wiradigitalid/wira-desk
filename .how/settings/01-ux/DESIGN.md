...
`DEC-014` carries this one step further: the `Layout` pane held exactly one control — `stack_width_percent`, never a chord — and once Overlapping Stack's own on/off toggle moved to `Shortcuts` (`SPEC-3-01`), a whole pane survived to hold one percentage field. That field moves inline onto the Overlapping Stack row instead, the same `has_percent` pattern the four `Snap custom` rows already use, and the `Layout` pane is retired rather than kept alive for a control that fits its sibling row's own shape.
