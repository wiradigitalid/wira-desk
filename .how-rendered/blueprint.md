...
| **Ring Buffer Command** | `Command` enum (`0..=15`) | `window-management` | `Hook Thread` → `Worker Thread` | In-process lock-free static ring buffer (`16` slots) | Raw `u8` byte (`0`=Nop, `1`=Cycle, `2`=SnapLeft, `3`=SnapRight, `4`=SnapMax, `5`=Stack, `6`=SnapTop, `7`=SnapBottom, `8`=MoveToNextMonitor, `9`-`12`=SnapPercentLeft/Right/Top/Bottom, `13`-`15`=SnapThirdLeft/Middle/Right) | Fire-and-forget command transfer with zero heap allocation. |
