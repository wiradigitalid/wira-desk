...
- **Enforcement:** The daemon owns task registration (`crates/daemon/src/autostart.rs`). Registration tries an XML definition first, falling back to plain `schtasks /Create /TN WiraDesk /TR "\"<exe_path>\"" /SC ONLOGON /RL HIGHEST /RU "%USERNAME%" /F` flags only when the XML path is unavailable (a staging directory that fails an ACL check, or a schema this code cannot validate locally). The XML path is the only one that corrects three settings `schtasks` gets wrong by default for a task registered this way: it would otherwise refuse to start on battery, get killed the moment the machine unplugs, and be killed again after 72 hours of running — all three wrong for a background daemon meant to just keep running. The flags-only fallback registers with those defaults intact rather than failing auto-start outright. The settings process never calls `schtasks`: it writes `general.auto_start` to `config.toml` and signals reload, and the daemon reconciles the task on reload. `schtasks /Query` is the authoritative source for the menu checkmark, not the config value — and it stays a pure existence check: keeping the stored path current is `BR-7`'s job, deliberately separated so the checkmark cannot go blank over a drifted path.
...
### BR-8 — The Update Check Is the Product's Only Network Activity

- **Rationale:** The product's whole privacy position rests on being able to say "no network connections" as an absolute — except for the one thing a user actually wants told to them without asking: whether a newer version exists. Carving out exactly that, and nothing riding along with it, is what lets the exception be stated in one sentence and verified by reading the request itself rather than trusted on the strength of who wrote it.
- **Enforcement:** `crates/daemon/src/updatecheck.rs` (`spawn`, `run_once`) runs the periodic check — first at 2 minutes after start, then every 24 hours, toggleable via `general.check_updates` in config. `crates/shared/src/https.rs` (`get_text`, `get_streaming`) is the only HTTP client in the product; `split_url` and its callers refuse anything that is not plain HTTPS, and the user agent (`USER_AGENT`) carries only the product name and version, no machine or user identifier. `crates/shared/src/update.rs` (`decide`) parses the small per-release descriptor, compares versions numerically, and validates the download URL is host- and path-pinned to the product's own release channel before ever offering it — a tampered descriptor pointing elsewhere is refused, not followed. `crates/settings/src/update.rs` (plus `sha256.rs`) is the user-triggered half: on confirmation it downloads the installer over the same HTTPS client, verifies the download's SHA-256 against the hash the descriptor published, deletes and refuses to launch anything that does not match, and only then launches the verified file elevated.
