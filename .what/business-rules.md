...
| BR-5 | System tray "View Logs" action opens the silent log folder location defined in platform runtime paths; the settings UI does not duplicate log inspection or file handling interfaces. | `window-management`, `settings` | FR-12, CAP-11, AD-1 | active |
...
- **Enforcement:** Settings shell and installer scripts register the Windows Scheduled Task using `schtasks /Create /TN WiraDeskAutoStart /TR "..." /SC ONLOGON /RL HIGHEST /RU %USERNAME% /F`.

### BR-5 — Single Responsibility for Diagnostic Log Access
- **Rationale:** Duplicate UI for log viewing inside the settings binary adds bloat and complicates UI navigation. Exposing log inspection directly through native Windows Explorer launched from the tray menu keeps the settings binary focused purely on preferences and onboarding, adhering to single-responsibility boundaries.
- **Enforcement:** The tray context menu "View Logs" handler invokes `ShellExecuteW(..., L"explore", log_dir, ...)`; settings UI omits log viewer components.
