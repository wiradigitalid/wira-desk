...
- [x] `ShortcutField::group()` returns exactly these five strings, and every field maps to the group
...
- [x] `ShortcutField::ALL`'s declared order changes exactly as `DEC-014` describes: `SnapMaximize` moves out
...
- [x] `ShortcutField::label()` for the four `SnapPercent*` fields drops the parenthetical: `"Snap to left
...
- [x] `crates/settings/src/main.rs`'s three `group_rows("...")` calls become five, matching the new group
...
- [x] `crates/settings/ui/panes/shortcuts_pane.slint` grows a `ShortcutGroup` per new group, heading text
      exactly matching `group()`'s five strings, in declared order.
- [x] The existing test asserting concatenating the pane groups reproduces `ShortcutField::ALL` exactly
...
- [x] A test asserts `SnapMaximize`'s new position relative to every `Snap*` field (its precedent test
...
- [x] A test asserts none of the four `Snap to custom` labels contain the substring `"(custom"` any more.
- [x] Any existing collision/precedence test that pins a specific winner between two default-config
...
- [x] Grep the corpus's own prose — `EXPERIENCE.md`, onboarding/tutorial copy, any tip or help string in
...
`load_shortcuts_from_config`'s row table in `crates/daemon/src/hook.rs`. They agree only by coincidence — nothing reads
...
- [x] `crates/shared/src/constants.rs` grows `SHORTCUT_DECLARED_ORDER`: the sixteen config keys in
...
- [x] `ShortcutField::ALL`'s keys equal `SHORTCUT_DECLARED_ORDER`, asserted
...
- [x] `Chords::in_declared_order` and `load_shortcuts_from_config`'s row table carry the same reorder, and the
...
- [x] `dec_011_collision_favors_percent_snap_bottom_over_legacy_stack_default` is the collision fixture the
...
- [x] **Derive `validate_config`'s field order from `shared::constants::SHORTCUT_DECLARED_ORDER`
...
- [x] `persistence::tests::a_duplicate_names_the_holder_by_the_shared_declared_order` goes green.
...
- [x] `main.rs` selects rows by indexing `ShortcutField::GROUPS` instead of passing its own string
...
- [x] Move `snap_maximize` and `move_next_monitor` to their new positions in the `Chords` **struct
