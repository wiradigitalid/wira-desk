...
- [ ] **This is an idea from the owner, not a decided design — read `DEF-15`'s `fix_direction` for the
...
- [ ] A row's rendered height is identical whether its toggle is on or off. A new snapshot test toggles one
      row both ways and asserts the row's rendered height is unchanged.
- [ ] The "Disabled" caption still renders, in the same place relative to the title, when its row's toggle is
      off — this ticket fixes the height reacting to it, not whether it appears at all.
- [ ] `SPEC-4-03`'s own row-height guard
...
- [ ] A slightly taller per-row height, applied across all sixteen rows, does not silently reintroduce
