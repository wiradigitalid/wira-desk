...
- [x] **Confirm the established root cause before changing anything**, per this repo's standing
      ...
      this row's own `z`.
- [x] Root-cause whether Slint's `PopupWindow` (a component built for exactly this shape — anchored
...
- [x] Whatever the mechanism, the tooltip renders as the topmost element at its own screen region
...
- [x] A new test renders at least two adjacent rows (or a row plus `KeyCheck`) together and asserts
...
- [x] `SPEC-5-01`'s existing tooltip guards
      ...
      positioning.
- [x] While fixing paint order, the builder MAY also make the tooltip's positioning cursor-aware
