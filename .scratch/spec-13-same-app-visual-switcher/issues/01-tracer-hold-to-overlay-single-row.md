...
- [ ] The eligible-collection half of `run_context_safe_cycle` is extracted into one function,
      and a test asserts the switcher's candidate set equals the blind cycle's eligible set for
      the same snapshot
- [ ] Key-down still enqueues `Cycle` with no added delay (the `SM-1` path is untouched) and
      arms a hold timer on the Worker keyed to the **main key**, not the modifier
- [ ] A main-key key-up before the deadline disarms; nothing is drawn and the tap is
      indistinguishable from today's behaviour
- [ ] The deadline reached with every modifier in the matched chord still physically down opens
      the overlay, selection on the window the key-down already activated
- [ ] Window class `WiraDeskVisualSwitcher`, `WS_POPUP` with
...
- [ ] `DwmThumbnailHandle` registers one source window and unregisters in `Drop`; a failed
      registration leaves an empty card and neither removes it nor aborts the overlay
- [ ] Cards are ordered by `cycle_order`, so `` ` `` always moves the selection to the visually
      next card; a test asserts the two orders agree
- [ ] New ring opcodes `SwitcherDisarm`, `SwitcherNext`, `SwitcherPrev`, `SwitcherCommit`,
      `SwitcherCancel` round-trip through `Command::from_u8`, and the navigation opcodes are
      exempt from `ANTI_MACRO_THROTTLE_MS`
- [ ] Every modifier in the matched chord released commits through the existing
...
- [ ] A chord with no modifier never arms the switcher (there is no release to commit on) —
      covered by a test over `Shortcut::has_modifier`
- [ ] A VM/RDP bypassed chord never arms the switcher, because arming happens after the bypass
      decision
- [ ] Watchdog: a `SetTimer` tick polling `GetAsyncKeyState` commits when the modifiers are
...
- [ ] Thumbnail registration and unregistration are counted through a `ThumbnailSink` trait with
      a counting fake; the leak guard is seen failing first (remove the `Drop` impl, watch the
      count go non-zero, restore it)
- [ ] All `unsafe` blocks carry `SAFETY:` comments stating the precondition relied on
