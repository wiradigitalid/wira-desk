...
- [ ] `cols = clamp(floor((usable + GUTTER) / (CARD_W + GUTTER)), 1, n)` with
      ...
      settings
- [ ] Each card yields **two** rectangles — a chrome box and a preview box — because a DWM
      thumbnail composes above the destination window and nothing may be drawn on top of it
- [ ] The preview box is uniform across cards; per-window aspect ratio is not computed, because
      DWM letterboxes the source inside the destination rectangle itself
- [ ] Page count and active page index, with the leaving page's thumbnails unregistered and the
      arriving page's registered on every turn — at most `per_page` live thumbnails at any
      moment
- [ ] Advancing past the last card on a page turns to the next page with the selection on its
      first card; retreating past the first card turns back to the previous page's last card
- [ ] Up and Down move one row within a page and clamp at the top and bottom rows
- [ ] Page indicator (`● ○ ○`) painted in the overlay's chrome area
- [ ] Layout is per-monitor DPI aware and is computed against the work area of the monitor
      holding the active window, not the primary monitor
- [ ] Unit tests cover 1, 2, 5, 8, 14, 20, 35 and 60 candidates against work-area widths of
