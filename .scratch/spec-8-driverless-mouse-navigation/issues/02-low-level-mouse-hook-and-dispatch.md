...
- [ ] `shared::Command` opcodes extended with new values:
...
- [ ] `HookSnapshot` and `HookRuntime` extended:
...
- [ ] `HookThread` installs both `WH_KEYBOARD_LL` and `WH_MOUSE_LL` during startup; `refresh_hook_on_hook_thread` verifies and refreshes both handles on 10s heartbeat checks.
- [ ] At the entry of `low_level_mouse_proc`:
...
- [ ] VM/RDP bypass check evaluated for auxiliary mouse inputs: if the foreground window matches `BypassPolicy`, calls `CallNextHookEx` immediately without interception.
- [ ] Input extraction and mapping:
...
- [ ] Worker execution in `crates/daemon/src/worker.rs`:
...
- [ ] Clean shutdown: `UnhookWindowsHookEx` called for both keyboard and mouse hook handles.
- [ ] Pure helper functions (`resolve_mouse_event`, `apply_tilt_debounce`, `map_preset_to_command`) extracted for unit testing.
- [ ] All unit tests pass in `crates/daemon`.
