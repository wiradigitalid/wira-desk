...
- [ ] `shared::Config` gains sixteen new boolean fields, one per `ShortcutField`, alongside the existing
...
- [ ] Every new field's `Default` arm sets `true` explicitly — not derived from the primitive `bool`
...
- [ ] `layout.enable_overlapping_stack` is removed from the schema; Overlapping Stack's row uses the new
...
- [ ] Loading a config that still has `layout.enable_overlapping_stack = false` seeds Overlapping Stack's
...
- [ ] `crates/settings/src/persistence.rs`'s `validate_config`/`find_conflict` exclude a disabled field's
...
- [ ] Disabling an action never touches its stored chord string; toggling it back on (with no other edit)
...
- [ ] A UI control appears on every Shortcuts-pane row to toggle the flag, wired to the model the same way
...
- [ ] Every new field round-trips through save/reload unchanged (existing round-trip test pattern extended).
- [ ] The frozen shipped-defaults test is updated: every action's default enabled state is `true`, and the
